...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['form_token']) || $_POST['form_token'] !== $_SESSION['form_token']) {
        ...
        // Check if the schedule is for Baptism
        if (stripos($details, 'baptism') !== false) {
            $time = '10:00'; // Set fixed time for Baptism schedules
        }
        ...
        if (empty($date) || empty($time) || empty($status) || empty($details)) {
            ...
            // Check the number of schedules for the selected date
            $query = "SELECT COUNT(*) AS count FROM available_schedule WHERE date = '$date'";
            $result = mysqli_query($conn, $query);
            $row = mysqli_fetch_assoc($result);

            if ($row['count'] >= 10) {
                $error = "Maximum limit of 10 schedules per date has been reached.";
            } else {
                ...
                if (mysqli_query($conn, $query)) {
                    ...
                    header("Location: adminAddSchedule.php?success=1");
                    exit();
                } else {
...
<head>
    ...
    <script>
        ...
        // Automatically set the time for Baptism
        function checkDetails() {
            const detailsInput = document.getElementById('details');
            const timeInput = document.getElementById('time');
            if (detailsInput.value.toLowerCase().includes('baptism')) {
                timeInput.value = '10:00'; // Fixed time for Baptism
    ...
    <div id="forWhite" class="container mt-5">
        ...
        <form method="POST" action="" onsubmit="disableSubmitButton()">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control" id="details" name="details" rows="3" required oninput="checkDetails()"></textarea>
            </div>
