...
function logActivity($conn, $username, $status) {
    ...
    $activity_date = date('Y-m-d'); // Current date in YYYY-MM-DD format
    $activity_time = date('H:i:s'); // Current time in HH:MM:SS format
    // Prepare the SQL statement to insert the log
    $stmt = $conn->prepare("INSERT INTO activity_log (username, status,appointment_date, appointment_time ) VALUES (?, ?,?,?)");
    $stmt->bind_param("ssss", $username, $status,$activity_date, $activity_time); // Bind parameters to the prepared statement
...
function verifyLogin($conn, $input, $password) {
    ...
    if ($result->num_rows === 1) {
        ...
        if ($isRestricted == 1) {
            logActivity($conn, $input, 'Account restricted'); // Log restricted account activity
            return 'Account is restricted. Please contact support.'; // Account is restricted
        }

        // Verify the entered password against the hashed password
        if (password_verify($password, $hashedPassword)) {
            logActivity($conn, $username, 'Successful login'); // Log successful login
            return $username; // Return the username if login is successful
        }
    }
    
    logActivity($conn, $input, 'Failed login'); // Log failed login attempt
