...
<div class="row align-items-center text-center my-5 mass-schedule-section">
    <div class="col-md-6">
        <h2 class="text-light">SET AN APPOINTMENT</h2>
        ...
        <button class="btn btn-secondary mt-4" onclick="massSchedule()">SET AN APPOINTMENT</button>
    </div>
    <div class="col-md-6">
...
<div class="modal fade" id="massScheduleModal" tabindex="-1" role="dialog" aria-labelledby="massScheduleModalLabel" aria-hidden="true">
    ...
    <div class="col-md-6">
        ...
        $events = $conn->query("SELECT * FROM events ORDER BY created_at DESC LIMIT 3"); // Fetch latest 3 events
    ...
    <div class="col-md-6">
    <h4 class="text-light">Saint of the Month</h4>
    <?php
    include '../config/connection.php'; // Database connection

    // Fetch the latest event
    $events = $conn->query("SELECT * FROM events ORDER BY created_at DESC LIMIT 1");
    if ($events && $events->num_rows > 0) {
        $event = $events->fetch_assoc();
        if (isset($event['image_path'])) {
            $image_path = "../Images/" . htmlspecialchars($event['image_path']);
            echo "<img src=\"$image_path\" class=\"img-fluid\" alt=\"Event Image\" style=\"width: 450px; height: 450px;\">";
        } else {
            echo "<p>No event image found.</p>";
        }
    } else {
    ...
    ?>
       ...
       <div class="row align-items-center text-center my-5 services-offered-section bg-white" style="padding : 20px;">
    <div class="col-12">
