<?php 
    ...
    function generateUniqueCode($conn) {
        do {
            $unique_code = '2024' . rand(100000, 999999);  // Generate a random 6-digit number after '2024'
            // Check if the code already exists in the database
            $stmt = $conn->prepare("SELECT id FROM user WHERE unique_code = ?");
            $stmt->bind_param("s", $unique_code);
            $stmt->execute();
            $stmt->store_result();
        } while ($stmt->num_rows > 0);  // Repeat if code exists
        $stmt->close();
        return $unique_code;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($password !== $confirm_password) {
            ...
            if ($stmt->num_rows > 0) {
                ...
                $stmt = $conn->prepare("INSERT INTO user (email, username, contact_num, password, unique_code) VALUES (?, ?, ?, ?, ?)");
                $stmt->bind_param("sssss", $email, $username, $contact_number, $hashedPassword, $unique_code);
