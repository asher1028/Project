...
function draw_calendar($month, $year, $schedules) {
    ...
    $startingDay = date('w', $firstDay); // Get the day of the week for the first day of the month
    ...
    // Adjust the starting day (empty cells before the first day of the month)
    ...
    for ($day = 1; $day <= $totalDays; $day++) {
        ...
        // Check if the current date is a Monday (1 = Monday)
        $isMonday = (date('w', strtotime($currentDate)) == 1);

        // Disable the Apply button for Mondays and past dates
        if ($isMonday) {
            $button = '<button class="btn btn-secondary btn-sm" disabled>Monday - Blocked</button>';
        } elseif ($currentDate < $today) {
            $button = '<button class="btn btn-secondary btn-sm" disabled>Past Date</button>';
        } elseif ($hasSchedule) {
...
<head>
    ...
    <link rel="stylesheet" href="userBaptismAvailableSchedule.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
    <script>
        // Function to apply for a selected schedule
        function applyForSchedule(date) {
            window.location.href = 'userBaptismApplication.php?date=' + date;
        }
    </script>
    <style>
        .today {
        ...
        /* Add custom styles for blocked dates */
...
<body>
    ...
    <div id="mainDiv">
        ...
        <div class="text-space-between">
            <a href="userBaptismAvailableSchedule.php?month=<?php echo ($month == 1 ? 12 : $month - 1); ?>&year=<?php echo ($month == 1 ? $year - 1 : $year); ?>" class="btn btn-secondary">Previous</a>
            <a href="userBaptismSchedule.php" class="btn btn-danger">Back</a>
            <a href="userBaptismAvailableSchedule.php?month=<?php echo ($month == 12 ? 1 : $month + 1); ?>&year=<?php echo ($month == 12 ? $year + 1 : $year); ?>" class="btn btn-secondary">Next</a>
        </div>
    </div>
</body>
</html>
